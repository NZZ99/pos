...
export const InventoryTab: React.FC<InventoryTabProps> = ({
  ...
  const inventoryData = products.map((p) => {
    ...
    const isLow = currentStock <= p.minStockKg;
                ...
                {filteredProducts.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="py-3.5 px-4 text-right border-r border-slate-200 text-slate-600">
                      {item.totalIn.toLocaleString()} KG
                    </td>
                    <td className="py-3.5 px-4 text-right border-r border-slate-200 text-slate-600">
                      {item.totalSold.toLocaleString()} KG
                    </td>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs sm:text-sm">
              <thead>
                <tr className="bg-indigo-700 text-white font-semibold tracking-wide">
                  <th className="py-3 px-4 text-center border-r border-indigo-600/50">Batch ID</th>
                  ...
                  <th className="py-3 px-3 text-right border-r border-indigo-600/50">အဝင် KG</th>
              ...
              <tbody className="divide-y divide-slate-200 text-slate-700">
                {stockInList.map((stk, idx) => (
                  ...
                  >
                    <td className="py-3 px-4 text-center font-bold text-indigo-700 border-r border-slate-200">
                      {stk.batchId}
                    </td>
                    ...
                    <td className="py-3 px-4 border-r border-slate-200 font-medium text-slate-900">
                      {stk.productName} ({stk.productCode})
                    </td>
                    <td className="py-3 px-3 text-right border-r border-slate-200 font-semibold text-slate-800">
                      {stk.totalKg.toLocaleString()} KG
                    </td>
                    <td className="py-3 px-3 text-center border-r border-slate-200">
                      <span className="inline-flex items-center gap-1 text-xs text-slate-600">
                        <Clock className="w-3 h-3 text-slate-400" />
                        <span>{stk.expiryDate}</span>
                      </span>
                    </td>
                    <td className="py-3 px-3 text-center border-r border-slate-200">
